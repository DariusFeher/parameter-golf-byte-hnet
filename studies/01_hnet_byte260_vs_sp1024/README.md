# [OpenAI-highlighted non-record] 1-Stage Byte-level H-Net at 17.5M: Dynamic Chunking Learns Whitespace-Aligned Boundaries

>OpenAI later highlighted this submission in its Parameter Golf retrospective as one of its three favorite non-record submissions:
 > - OpenAI Blog Post, May 2026: [What Parameter Golf taught us
](https://openai.com/index/what-parameter-golf-taught-us/)
 > - Byte HNet - Parameter Golf - Github Repo: [Code](https://github.com/openai/parameter-golf/tree/main/records/track_non_record_16mb/2026-03-29_HNet_ByteVsSubword_Study)

In this non-record submission I study how H-Net's dynamic chunking compares on **raw bytes (`byte260`)** vs **pre-tokenized subwords (`sp1024`)**, using the same 1-stage 9-layer H-Net backbone and the challenge’s tokenizer-agnostic **BPB** metric. This work is based on **Hwang et al. (2024), [Dynamic Chunking for End-to-End Hierarchical Sequence Modeling](https://arxiv.org/abs/2507.07955)**, 
...
- a matched `byte260` vs `sp1024` H-Net ablation study
...
## Key Results (≤16MB)
 ...
 | Config | BPB | Size (MB) | Steps | Training time | Seeds |
  ...
  | sp1024 10-min (chunk=12, KV=2) | 1.3734 | 15.99 | 4,466 | 10 min | 1337 |
  |**Official Baselines - Subword tokenization, no H-Net**||
  | 10-min | 1.2244 | 15.9 | 13,780 | 10 min | N/A |
  | 4-hour | 1.2074 | 15.8 | 329,430 | 4 hours | N/A |
...
> Check **Legend** Section for abbreviation meanings.
...
- The `sp1024` chunker splits mid-word / beginning of the word at subword token boundaries (`[I][nsurance Com][pany De][clares]` or `G][eorge`]) and alternates between 1-token fragments and 20+ token spans. Chunk size CV is 0.76 (`sp1024`) vs 0.45 (`byte260`), confirming quantitatively that byte boundaries are more consistent in terms of chunk size uniformity: the `byte260` model produces chunks of similar length (word-scale), while `sp1024` oscillates between single-token fragments and long multi-token spans.
...
DATA_PATH=./data/byte260_export/datasets/fineweb10B_byte260 \
    ...
    HNET_LR_DIFF=0.75 torchrun --standalone --nproc_per_node=8 \
    studies/01_hnet_byte260_vs_sp1024/train_gpt_hnet_byte260.py
...
MAX_WALLCLOCK_SECONDS=600 NUM_LAYERS=9 MODEL_DIM=512 \
    ...
    HNET_LR_DIFF=0.75 NUM_KV_HEADS=2 torchrun --standalone --nproc_per_node=8 \
    studies/01_hnet_byte260_vs_sp1024/train_gpt_hnet_sp1024.py
```
...
- **Paper**: Hwang et al. (2024), [*Dynamic Chunking for End-to-End Hierarchical Sequence Modeling*](https://arxiv.org/abs/2507.07955) — the H-Net architecture this submission implements. Official code: [github.com/goombalab/hnet](https://github.com/goombalab/hnet)
